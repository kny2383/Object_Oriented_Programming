...
void Student::Modify() { // 정보 수정
	Subject* findSub = NULL; // 찾고자하는 과목 이름
	string modSubName; // 과목 이름
	...
	//과목 정보를 입력한 경우
		...
		cout << "수정하려는 과목의 이름을 입력하세요 : ";
		InputValue(modSubName);
		findSub = SubSearch(modSubName); // 입력한 과목 이름이 수강하는 과목과 일치하는지 비교
		...
		if (findSub != NULL) {
			...
			cout << "검색한 과목을 수강하지 않습니다. 다시 검색하세요.\n";
		}
	}
	else if (Type == "모두") {
		StdModify(); // 학생 정보 수정
